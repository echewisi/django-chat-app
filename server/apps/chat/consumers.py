...
class ChatConsumer(AsyncWebsocketConsumer):
	...
	def saveMessage(self, message, userId, roomId, read=False):
			userObj = User.objects.get(id=userId)
			chatObj = ChatRoom.objects.get(roomId=roomId)
			chatMessageObj = ChatMessage.objects.create(
				chat=chatObj, user=userObj, message=message, read=read
			)
			return {
				'action': 'message',
				'user': userId,
				...
				'read': read  # Add the 'read' field to the response
			}
	...
	async def update_read_receipt(self, message_id):
			try:
				message = ChatMessage.objects.get(id=message_id)
				if message.user != self.user and not message.read:
					message.read = True
